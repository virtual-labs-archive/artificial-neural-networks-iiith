...
void drawOPTIONS()
{
  ...
  if (errorCHIOCE==true)
  {
    fill(0, 0, 255);
    textSize(20);
    text("< WRONG CHOICE::PRESS BUTTON TO ENTER AGAIN >", 70, 150);
    noFill();  
    fill(50);
    rect(620, 130, 70, 25);
    fill(0);
  ...
  if (stageTWOshow == false && stageTHREEshow == false)
  {
    ...
    text("MOVE THE MOUSE OVER THE STATES AND CLICK TO CHOOSE AS MINIMUM", 40, 210);
    text("YOU CAN CHOOSE AT MOST TWO STATES AS MINIMUM", 130, 240); 
    text("MAKE SURE THEY ARE SAPERATED BY MORE THAN ONE HAMMING DISTANCE", 30, 270);
    createDIAG();
  }
  else if (stageTHREEshow == true)
  {
    ...
    fill(0);
    textSize(20);
    text("CLICK HERE", 55, 405);
    text("CLICK HERE", 55, 475);
    textSize(16);
    text("TO GO BACK TO ADJUST WEIGHTS", 15, 435);
    text("TO GO BACK TO CHOOSE STATES", 15, 510);
...
void drawENERGYdiagram(staTES[] stateARRR, int[] orderSTATESS)
{
  ...
  for (int i=0;i<orderSTATESS.length;i++)
  {
    ...
    for (int y=0;y<i;y++)
    {
      ...
      fill(255);
      textSize(15);
      text(tt[0], drawStartXX+40, (drawStartY-(65*in))+20);
