function coreMlp(numSamplesPerClass, numIterations, sampleFlag,  iterationFlag, sampleStep, iterationStep, linearity_flag)
...
if ( (iterationFlag==0) & (sampleFlag == 0) )
	[X, X1, X2, w, W, P, T, N, d, r, eta0, etaT, indexArray, currentSampleIndex, trainingSample, currentIterationIndex, globalCount, updateFlag] = initMlp(numSamplesPerClass, numIterations, linearity_flag);
	...
	if globalCount == 1
		...
		hold on;
		plot(X2(:,1), X2(:,2), 'rv', "markersize", 10);
		...
		plot(trainingSample(1), trainingSample(2), 'ko', "markersize", 15,'linewidth',2);
		%title(['Iteration #: ' num2str(currentIterationIndex) ' ; Sample #: ' num2str(currentSampleIndex)]);
