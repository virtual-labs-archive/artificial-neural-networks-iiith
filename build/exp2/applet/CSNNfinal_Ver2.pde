...
void testCircle()
{
  ...
  do
  {
    for (int i=0;i<40;i+=1)
    {
      for (int j=0;j<40;j+=1)
      {
        StatesNext[i] = StatesNext[i]+StatesInit[j]*tempI[i][j];
      }
    }
    ...
    for (int i=0;i<40;i+=1)
    {
      ...
      float fTEMP = StatesNext[i];
      StatesNext[i] = (exp(2*(StatesNext[i]))-1)/( exp(2*(StatesNext[i]))+1);
      StatesNext[i] = Round(StatesNext[i],3);
      println("_____"+StatesNext[i]+"______"+fTEMP);
      
      if (stage4MARK[i]==false)
      {
    ...
    {
        valueOfIter[countdowhile][i1] = abs(StatesInit[i1]);
//      valueOfIter[countdowhile][i1] = StatesNext[i1];
//    if(stage4MARK[i1]==true)
if(i1==34)
    {
//    print(valueOfIter[countdowhile][i1]+"___"+countdowhile+"__"+i1+"  *****  ");  
    }
  }println("LINE CHANGE");
    countdowhile+=1;
  }
...
void displayITER()
{
  if (stage ==4 && flagDES==true)
  {
    ...
    for (int i = 0;i<20;i+=1)
    {
      ...
      {
        stroke(0);
        int f1 = int(valueOfIter[10+j][i]*120);
        int f2 = int(valueOfIter[10+j][20+i]*120);
        fill(f1);
